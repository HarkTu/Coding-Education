...
public final class Team7
{

  public static void greetings()
  {
    final String message = " With love by Team-7 ";
    final int n = 10;
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j <= 4 * n; j++) {
        ...
        if (d1 < n + 0.5 || d2 < n + 0.5) {
          System.out.print(TEXT_BRIGHT_RED + '*');
        }
        else {
          System.out.print(' ');
        }
      ...
      for (int j = 0; j < 4 * n + 1 - 2 * i; j++) {
        if (i >= 2 && i <= 4) {
          ...
          if (idx < message.length() && idx >= 0) {
            if (i == 3) {
              System.out.print(COLOUR_BLUE + TEXT_BRIGHT_BG_WHITE + message.charAt(idx) + COLOUR_RESET);
            }
            else {
              System.out.print(TEXT_BRIGHT_BG_WHITE + ' ' + COLOUR_RESET);
            }
          }
          else {
